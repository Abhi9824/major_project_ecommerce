...
const Checkout = () => {
  ...
  const {addresses,error, status,addressAdded}=useSelector((state)=>state.address)
...
dispatch(fetchCartProducts())
  ...
  <div>
    ...
    <div className='d-flex justify-content-between'>
      ...
      <button className='addAdrres' onClick={() => { setEditAddress(null); setShowForm(true); }}>ADD NEW ADDRESS</button>
    </div>
    <p className='fontSize'>DEFAULT ADDRESS</p>
    <div className="address-container">
            {addresses?.map((address) => (
              <div key={address._id} className="address-card card mb-2 py-4 mt-4">
               ...
               <div>
                  ...
                  <p>{address.address?.street}<br />
                    {address.address?.city}, {address.address?.state} - {address.address?.pinCode}
                  </p>
                  <p>Mobile: <b>{address.contact?.phoneNumber}</b></p>
                  <div className="d-flex">
                    <button className="btn edit rounded"onClick={()=>editHandler(address)}>Edit</button>
                    <button className="btn remove rounded" onClick={()=>removeHandler(address._id)}>Remove</button>
                  </div>
                  </div>
                </div>
              </div>
    ...
    <div className="col-md-4 ms-4 ">
            <div className="card priceCard px-3 d-flex">
                ...
                <Link to={`/checkout`}>
                    <button className="btn placeOrder mt-2 w-100 py-2">Place Order </button>
                </Link>
            </div>
        </div>
